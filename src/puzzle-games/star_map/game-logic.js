/**
 * STAR MAP - GAME LOGIC (IFRAME VERSION)
 */

const gameContainer = document.getElementById('game-container');

function shellStatus(text, html = false) {
    window.parent.postMessage({ type: 'SET_STATUS', data: { text, html } }, '*');
}

function shellLog(message, prefix = 'SYNC', color = '#70d1f4') {
    window.parent.postMessage({ type: 'LOG_MESSAGE', data: { message, prefix, color } }, '*');
}

function initGame(container) {
    const scene = new THREE.Scene();
    // Transparent background so the shell CSS shows through if needed, 
    // but here we keep the dark space look.
        ...
        if (intersects.length > 0) {
            ...
            if (data.isTarget) {
                ...
                if (winStatus === "WIN") {
                    ...
                    isGameWon = true; 
                    shellStatus(`FAILURE: UNSTABLE_LINK <button onclick="window.parent.postMessage({type:'RELOAD_SHELL'}, '*')" style="margin-left:10px; background:none; border:1px solid #ff4444; color:#ff4444; cursor:pointer; font-family:inherit; font-size:0.7rem; padding: 2px 5px;">[RESTART]</button>`, true);
...
// Start
shellStatus('TRIANGULATION_IN_PROGRESS');
